                                              ...
                                              orte_jobid_t job)
{
    ...
    if (ORTE_SUCCESS != (rc = opal_dss.pack(data, &jdata->peer_mig_modex, 1, ORTE_GRPCOMM_COLL_ID_T))) {
        ORTE_ERROR_LOG(rc);
        return rc;
    }
#endif
                                                ...
                                                orte_jobid_t *job)
{
    ...
    *job = ORTE_JOBID_INVALID;
    ...
    cnt=1;
    if (ORTE_SUCCESS != (rc = opal_dss.unpack(data, &jdata->peer_mig_modex, &cnt, ORTE_GRPCOMM_COLL_ID_T))) {
        ORTE_ERROR_LOG(rc);
        return rc;
    }
