...
static void send_cmd(orte_job_t *jdata);
...
static int init(void){
    ...
    opal_event_set(orte_event_base, &recv_ev, socket, OPAL_EV_READ, recv_data, NULL);
...
static int recv_data(int fd, short args, void *cbdata)
{
    ...
    if(false==cmd_received)
    {
        if(0>read(socket,response_cmd,sizeof(local_bbq_cmd_t)))
        {
    ...
    {
        if(0>read(socket,response_host,sizeof(local_bbq_res_item_t)))
        {
...
static int orte_ras_bbq_allocate(orte_job_t *jdata, opal_list_t *nodes)
{
    ...
    return ORTE_ERROR_ALLOCATION_PENDING;
}
...
static int send_cmd(orte_job_t *jdata)
{
    ...
    if(0>send(socket,command,sizeof(local_bbq_cmd_t)))
    {
    ...
    if(0>send(socket,job,sizeof(local_bbq_job_t)))
    {
